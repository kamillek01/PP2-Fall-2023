...
test = "бальзам_ополаскиватель_для_длител_ухода_за_окрашенными_волосами"
outcome = snake_to_camel(test)
print(outcome)

#8
txt = "ОператорФискальныхДанных"
print(re.findall('[А-Я][^А-Я]*', txt))

#9
def space(txt):
    string = re.sub(r'([а-я])([А-Я])', r'\1 \2', txt)
    return string
...
def camel_to_snake(txt):
        import re
        str1 = re.sub('(.)([А-Я][а-я]+)', r'\1_\2', txt)
        return re.sub('([а-я0-9])([А-Я])', r'\1_\2', str1).lower()

print(camel_to_snake('НомерБанковскойКарты'))